...
Then one line in `scripts/cli/train.py`'s `_register_builtins()` makes it
discoverable.
...
Edit [`scripts/cli/train.py`](https://github.com/microsoft/SkillOpt/blob/main/scripts/cli/train.py)
...
the registry lives in `scripts/cli/train.py` and is populated lazily so that
...
python scripts/cli/train.py --config configs/docfaithful/default.yaml
```
