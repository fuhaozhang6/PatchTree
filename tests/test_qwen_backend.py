...
def test_chat_target_explicitly_disables_thinking(
    monkeypatch: pytest.MonkeyPatch,
    ...
    assert recorder.calls[0]["payload"]["chat_template_kwargs"] == {
...
def test_configure_qwen_chat_runtime_toggle_controls_payload(
    ...
    assert recorder.calls[0]["payload"]["chat_template_kwargs"] == {"enable_thinking": True}
    assert recorder.calls[1]["payload"]["chat_template_kwargs"] == {
        "enable_thinking": False,
    }
