...
def _is_ssrf_safe_host(host: str) -> bool:
    """Reject SSRF targets that are never a legitimate vLLM/OpenAI endpoint.

    Self-use policy: loopback and private ranges stay allowed so a local or
    intranet vLLM server (e.g. http://localhost:8000/v1) keeps working. What we
    block is the classic SSRF surface — cloud metadata / link-local
    (169.254.0.0/16, fe80::/10), multicast, reserved, and unspecified addresses.
    All resolved addresses must pass; a host that resolves to any blocked
    address is rejected.
    """
    try:
    ...
    for *_, sockaddr in infos:
        try:
            ...
            return False
        # Self-use: loopback (incl. IPv6 ::1, which is also is_reserved) is
        # allowed. Order matters — 169.254.169.254 and 0.0.0.0 both report
        # is_private=True, so the dangerous-range block must come before any
        # private allow, otherwise the cloud-metadata IP would slip through.
        if ip.is_loopback:
            continue
...
def _can_connect_to_url(url: str, timeout: float = 0.5) -> bool:
    ...
    if not host or parsed.scheme not in ("http", "https"):
        return False
    if not _is_ssrf_safe_host(host):
        return False
...
class TrainingManager:
    ...
    def start(self, config_path: str, overrides: dict) -> str:
        ...
        cmd = [
            sys.executable, "scripts/cli/train.py",
            "--config", config_path,
        ]
        cfg_options = []
