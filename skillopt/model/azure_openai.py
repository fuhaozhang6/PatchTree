...
def _is_deepseek_official_chat(client: AzureOpenAI | OpenAI, deployment: str) -> bool:
    base_url = str(getattr(client, "base_url", "")).lower()
    return "api.deepseek.com" in base_url and deployment.lower().startswith("deepseek-")


def _chat_token_limit_kwargs(
    client: AzureOpenAI | OpenAI,
    deployment: str,
    max_completion_tokens: int,
) -> dict[str, int]:
    if _is_deepseek_official_chat(client, deployment):
        return {"max_tokens": max_completion_tokens}
    return {"max_completion_tokens": max_completion_tokens}


def _deepseek_extra_body(deployment: str) -> dict[str, Any] | None:
    if not deployment.lower().startswith("deepseek-"):
        return None
    value = (
        os.environ.get("DEEPSEEK_THINKING")
        or os.environ.get("DEEPSEEK_OFFICIAL_THINKING")
        or ""
    ).strip().lower()
    ...
    for attempt in range(retries):
        try:
            if _needs_responses_api(deployment):
                ...
                kwargs.update(_chat_token_limit_kwargs(client, deployment, max_completion_tokens))
                extra_body = _deepseek_extra_body(deployment)
                if extra_body:
