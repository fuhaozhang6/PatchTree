...
class QwenNonRetryableError(RuntimeError):
    """A deterministic request error that should not be retried."""
...
def _post_chat_completion(
    ...
    try:
        ...
        message = f"Qwen chat API returned HTTP {e.code}: {body}"
        if 400 <= int(e.code) < 500:
            raise QwenNonRetryableError(message) from e
...
def _chat_messages_impl(
    ...
    *,
    ...
    rollout_retries = os.environ.get(f"{role.upper()}_QWEN_CHAT_ROLLOUT_RETRIES") or os.environ.get("QWEN_CHAT_ROLLOUT_RETRIES")
    if str(stage or "").strip().lower() == "rollout" and rollout_retries:
        retries = max(1, min(int(retries), int(rollout_retries)))
    payload: dict[str, Any] = {
