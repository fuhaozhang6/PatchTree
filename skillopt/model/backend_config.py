...
def set_optimizer_backend(backend: str) -> None:
    ...
    normalized = normalize_backend_name(backend or "openai_chat")
    if normalized not in {"openai_chat", "claude_chat", "qwen_chat"}:
        raise ValueError(
            f"Unsupported optimizer backend: {normalized!r}. "
            "Supported values are 'openai_chat', 'claude_chat', and 'qwen_chat'. "
            "MiniMax is currently implemented only as a target backend."
        )
...
def set_target_backend(backend: str) -> None:
    ...
    normalized = normalize_backend_name(backend or "openai_chat")
    if normalized not in {"openai_chat", "claude_chat", "qwen_chat", "minimax_chat", "codex_exec", "claude_code_exec"}:
        raise ValueError(
            f"Unsupported target backend: {normalized!r}. "
...
def is_optimizer_chat_backend() -> bool:
    return OPTIMIZER_BACKEND in {"openai_chat", "claude_chat", "qwen_chat"}
