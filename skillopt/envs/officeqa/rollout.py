...
# Tool-capable small models (Doubao/Qwen) often leak template control tokens
# (e.g. <|FunctionCallEnd|>, </think>) into the content and label the final
# value in plain text instead of emitting an <answer> tag. These help recover it.
_TEXT_TOOL_CALL_RE = re.compile(r"<tool_call>(.*?)</tool_call>", re.IGNORECASE | re.DOTALL)
_TEXT_TOOL_FUNCTION_RE = re.compile(r"<function=([A-Za-z_][\w-]*)>(.*?)</function>", re.IGNORECASE | re.DOTALL)
_TEXT_TOOL_PARAMETER_RE = re.compile(r"<parameter=([^>]+)>(.*?)</parameter>", re.IGNORECASE | re.DOTALL)
_CONTROL_TOKEN_RE = re.compile(
    r"<\|[^|>]*\|>|</?think>|</?tool_call>|</?function[^>]*>|</?parameter[^>]*>",
    re.IGNORECASE,
)
_ANSWER_LABEL_RE = re.compile(r"^\**\s*(?:最终答案|final\s+answer|answer)\s*[:：]?\s*\**\s*", re.IGNORECASE)
...
def _decode_tool_arguments(arguments: object) -> dict:
    """Decode model-generated tool arguments without aborting an OfficeQA item."""
    ...
    if not isinstance(decoded, dict):
        raise ValueError(f"tool arguments are not a JSON object: {raw[:240]}")
    return decoded
...
def _extract_answer(text: str) -> str:
    ...
    if match:
        return _CONTROL_TOKEN_RE.sub(" ", match.group(1)).strip()
    cleaned = _TEXT_TOOL_CALL_RE.sub(" ", text or "")
    cleaned = _CONTROL_TOKEN_RE.sub(" ", cleaned)
    lines = [line.strip() for line in cleaned.splitlines() if line.strip()]
    if not lines:
        return cleaned.strip()
    last = lines[-1]
    stripped = _ANSWER_LABEL_RE.sub("", last).strip()
    return stripped or last


def _coerce_tool_argument(value: str) -> object:
...
def _parse_text_tool_calls(text: str) -> list[tuple[str, dict]]:
    """Parse Qwen XML-style tool calls emitted in message.content."""
    calls: list[tuple[str, dict]] = []
    for block_match in _TEXT_TOOL_CALL_RE.finditer(text or ""):
        block = block_match.group(1) or ""
        for fn_match in _TEXT_TOOL_FUNCTION_RE.finditer(block):
            name = fn_match.group(1).strip()
            body = fn_match.group(2) or ""
            args: dict[str, object] = {}
            for param_match in _TEXT_TOOL_PARAMETER_RE.finditer(body):
                key = param_match.group(1).strip()
                val = param_match.group(2).strip()
                if key:
                    args[key] = _coerce_tool_argument(val)
            if name:
...
def _build_codex_skill(skill_content: str) -> str:
    return render_skill_md(
        skill_content,
        description="Dynamic PatchTree skill for solving the current OfficeQA local-document question.",
        preamble=(
                ...
                if getattr(message, "tool_calls", None):
                    for tool_call in message.tool_calls:
                        ...
                        try:
                            ...
                            # vLLM auto tool choice parses arguments from model
                            # output but does not guarantee schema-valid JSON.
                            # Feed the error back as a tool result so the model
                            # can repair the call on its next turn instead of
                            # converting the whole sample into a hard-zero.
                            cmd = f"{tool_name}(invalid arguments)"
                    ...
                    continue
                text_tool_calls = _parse_text_tool_calls(response)
                if text_tool_calls:
                    for tool_name, arguments in text_tool_calls:
                        cmd, obs = run_tool(tool_name, arguments, allowed_roots=docs_roots, allowed_files=allowed_files)
                        conversation.append({"type": "tool_call", "cmd": cmd, "obs": obs, "format": "text_tool_call"})
                        messages.append({
                            "role": "user",
                            "content": (
                                f"## Tool Result\n{cmd}\n\n{obs}\n\n"
                                "Continue from this evidence. Return <answer>...</answer> if ready; "
                                "otherwise request another tool."
                            ),
                        })
                    continue
                if "<answer>" in response.lower():
                    final_answer = _extract_answer(response)
                    break
                # No tool call and no <answer> tag: the model produced a
                # free-form final message. Tool-capable small models often
                # label the value in plain text instead of tagging it, so
                # recover it here rather than discarding a real answer.
                recovered = _extract_answer(response)
                if recovered:
