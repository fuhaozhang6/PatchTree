...
*research* training/eval loops from this repo (``scripts/cli/train.py`` /
``scripts/cli/eval_only.py``) against the benchmark configs in ``configs/``.
...
TOOLS = [
    ...
    {
        "name": "skillopt_eval",
        "description": "Evaluate a single skill markdown file on a dataset without training (scripts/cli/eval_only.py).",
    },
]
