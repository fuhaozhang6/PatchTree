...
def adopt(night: str = None) -> int:
    ...
    # Read proposed skill from staging. Older OpenClaw staging manifests wrote
    # proposed_skill directly; the shared SkillOpt-Sleep staging layer writes
    # proposed_SKILL.md plus has_skill instead.
    manifest = Path(staging) / "manifest.json"
    if manifest.exists():
        with open(manifest) as f:
            m = json.load(f)
        proposed_raw = m.get("proposed_skill")
        proposed = Path(proposed_raw) if proposed_raw else None
        if proposed is not None and not proposed.is_absolute():
            proposed = Path(staging) / proposed
        if proposed is None and m.get("has_skill"):
            proposed = Path(staging) / "proposed_SKILL.md"
