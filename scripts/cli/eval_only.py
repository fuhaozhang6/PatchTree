#!/usr/bin/env python3
"""PatchTree eval-only: run a single skill on a dataset without training.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PatchTree eval-only")
...
def main() -> None:
    ...
    if explicit_backend is not None:
        ...
        if backend in {"claude", "claude_chat"}:
            ...
            cfg.setdefault("optimizer_backend", "openai_chat")
            cfg.setdefault("target_backend", "qwen_chat")
