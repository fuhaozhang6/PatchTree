...
# Launch SkillOpt-Tree LiveMathematicianBench training (cross-cloud).
#
# Default role split:
#   target / student    = Doubao Seed 1.6 Flash (Volcano Ark, doubao-seed-1-6-flash-250828)
#   optimizer / teacher = Qwen3.7-Plus (Aliyun DashScope, qwen3.7-plus)
#
# Target hits Volcano Ark (ARK_API_KEY); optimizer hits Aliyun DashScope
# compatible-mode (DASHSCOPE_API_KEY / ALI_API_KEY). Both are OpenAI-compatible.
...
# Optimizer runs on Aliyun DashScope (OpenAI compatible-mode), so it overrides
# the Ark endpoint/key inherited above. Set DASHSCOPE_API_KEY (or ALI_API_KEY).
export OPTIMIZER_AZURE_OPENAI_ENDPOINT="${OPTIMIZER_AZURE_OPENAI_ENDPOINT:-https://dashscope.aliyuncs.com/compatible-mode/v1}"
export OPTIMIZER_AZURE_OPENAI_API_KEY="${OPTIMIZER_AZURE_OPENAI_API_KEY:-${DASHSCOPE_API_KEY:-${ALI_API_KEY:-}}}"
export OPTIMIZER_AZURE_OPENAI_AUTH_MODE="${OPTIMIZER_AZURE_OPENAI_AUTH_MODE:-openai_compatible}"
...
OPTIMIZER_MODEL="${OPTIMIZER_MODEL:-qwen3.7-plus}"
