...
RUN_GROUP="${RUN_GROUP:-livemath_v3_ablation_$(date +%Y%m%d_%H%M%S)}"
...
# Keep these aligned with the 2x128 script unless the caller explicitly
# overrides them. The dataset-specific values below make this a single-dataset
# ablation instead of the original four-dataset run.
...
stop_vllm_from_first_run() {
  local pid_file="${PROJECT_ROOT}/logs/skillopt_v3_deepseek_local_qwen_parallel_${RUN_GROUP}_e1_b32_r3/vllm.pid"
...
# Run both ablations concurrently while sharing one vLLM endpoint. The first
# launcher starts Qwen and leaves it alive; the second launcher starts only
# after the endpoint is ready and never starts a second vLLM.
run_one "e1_b32_r3" 32 3 "${START_VLLM_FIRST:-1}" 0 &
first_pid=$!

wait_for_endpoint_or_first_exit "${first_pid}"

run_one "e1_b16_r1" 16 1 0 0 &
second_pid=$!
...
if [[ "${status_first}" -ne 0 || "${status_second}" -ne 0 ]]; then
  echo "[error] ablation failures: e1_b32_r3=${status_first}, e1_b16_r1=${status_second}" >&2
  exit 1
fi

echo "============================================================"
echo "  LiveMath V3 ablation group finished: ${RUN_GROUP}"
echo "============================================================"
